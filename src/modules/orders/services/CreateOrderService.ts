...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    for (let index = 0; index < foundProducts.length; index += 1) {
      if (products[index].quantity > foundProducts[index].quantity) {
        throw new AppError('Product with insuficient quantities');
      }
    }
